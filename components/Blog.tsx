    ...
    <section className="bg-white  py-20">
      <div className="py-8 px-4 mx-auto max-w-screen-xl lg:py-16 lg:px-6">
        ...
        <div className="grid gap-8 lg:grid-cols-2">
          {blogSheet.posts.map((post, index) => (
            <article
              key={index}
            ...
            >
              <div className="flex justify-between items-center mb-5 text-gray-500">
                ...
                {post.state === "testing" && (
                  <span className="w-3.5 h-2.5 bg-testing rounded-full"></span>
                )}
                {post.state === "completed" && (
                  <span className="w-3.5 h-2.5 bg-completed rounded-full"></span>
                )}
                {post.state === "checked" && (
                  <span className="w-3.5 h-2.5 bg-checked rounded-full"></span>
                )}
                {post.state === "warning" && (
                  <span className="w-3.5 h-2.5 bg-warning rounded-full"></span>
                )}
              </div>
